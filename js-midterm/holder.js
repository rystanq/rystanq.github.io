...
function random(min, max) {
  ...
  class Ball{
      ...
      upone(){
       volume = this.num;
      }// maybe issue animating, slow everything down 
...
const throttledClickHandler = throttle(function(event) {
    ...
    for (const ball of balls) {
        if (mouseX >= ball.x - ball.size && mouseX <= ball.x + ball.size &&
            mouseY >= ball.y - ball.size && mouseY <= ball.y + ball.size) {
            // If the mouse coordinates are within the bounds of the ball,
            // call the upone() method of the clicked ball to update the volume
            ball.upone();
            break; // Exit the loop since we found the clicked ball
        }
    }
